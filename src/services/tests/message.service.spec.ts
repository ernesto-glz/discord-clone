...
describe('GetAllInRoom', () => {
  it('When room not found -> Should throw ApiError', async () => {
    ...
    const error = await catchError(async () =>
      MessageService.getAllInRoom('1', '1', 30, 0)
    );
  ...
  it('When no messages found -> Should throw ApiError', async () => {
    ...
    const mockFindMessagesNull = jest.fn(() =>
      Promise.resolve(mockMessagesResult)
    );
    jest
      .spyOn(RoomRepository.prototype, 'findOne')
      .mockImplementation(mockFindRoomNull);
    jest
      .spyOn(MessageRepository.prototype, 'paginate')
      .mockImplementation(mockFindMessagesNull);
