...
describe('CreateFriendRequest', () => {
  it('When you try to friend yourself -> Should throw ApiError', async () => {
    ...
    try {
      ...
      expect(e.message).toEqual(ApiResponses.ERROR_CREATE_REQUEST);
    }
  });
  ...
  it('When they are already friends -> Should throw ApiError', async () => {
    ...
    try {
      ...
      expect(mockFindUser).toHaveBeenCalledTimes(1);
      expect(e.message).toEqual(ApiResponses.ALREADY_FRIENDS);
    }
  });
  ...
  it('When request already exists -> Should throw ApiError', async () => {
    ...
    try {
      ...
      expect(mockFindUser).toHaveBeenCalledTimes(1);
      expect(e.message).toEqual(ApiResponses.REQUEST_ALREADY_EXISTS);
    }
  });
});
...
describe('DeleteFriendRequest', () => {
  it('When try to delete a request that doesn\'t exist', async () => {
    ...
    try {
      await FriendService.deleteFriendRequest('10', '1');
    } catch (e: any) {
      expect(mockFindRequest).toHaveBeenCalledTimes(1);
      expect(e.message).toEqual(ApiResponses.REQUEST_NOT_FOUND);
    }
  });
});
...
describe('AcceptFriendRequest', () => {
  it('When try to accept a request that doesn\'t exist', async () => {
    ...
    jest.spyOn(FriendRepository.prototype, 'findOne').mockImplementation(mockFindRequest);
    try {
      await FriendService.acceptFriendRequest('10', '1');
    } catch (e: any) {
      expect(mockFindRequest).toHaveBeenCalledTimes(1);
      expect(e.message).toEqual(ApiResponses.REQUEST_NOT_FOUND);
    }
