...
export class UserService {
  public static async getUser(userId: string) {
    const foundUser = await deps.users.findById(userId);
    if (!foundUser) {
  ...
  public static setUserStatus = async (userId: string, status: 'ONLINE' | 'OFFLINE') => {
    ...
    return await deps.users.updateOne({ _id: userId }, { status });
  };

  public static setInHiddenDMS = async (userId: string, channelId: string) => {
    const result = await deps.users.updateOne({ _id: userId }, { $push: { hiddenDMChannels: channelId } });
    return result;
  };
